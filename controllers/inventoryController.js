...
exports.singleInventory = async (req, res) => {
    try {
        const data = await knex('inventories').where({ item_name: req.params.name});
        const filter= await data.filter(item=>item.warehouse_id===req.params.id)
        if (!data.length) {
            return res.status(404).send(`Record with id: ${req.params.name} is not found`);
        }
    
        res.status(200).json(data);
    } catch (err) {
        res.status(400).send(`Error retrieving inventory ${req.params.name} ${err}`)
    }
};